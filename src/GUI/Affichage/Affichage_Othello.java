 ...
 * @see Affichage
 */
public class Affichage_Othello extends Affichage {

    /**
     *  Constructeur d'<i>Affichage_Othello</i>, qui va initialiser la classe
     * mere et les images necessaires au jeu, et en fait une copie.
     *
     * @param panel JPanel sur lequel le jeu sera affiche.
     */
    public Affichage_Othello(JPanel panel) {
        super(panel);

        vide = new ImageIcon("./src/GUI/Affichage/Othello/vide.gif");

        pion = new ImageIcon[2];
        pion[0] = new ImageIcon("./src/GUI/Affichage/Othello/0.gif");
        pion[1] = new ImageIcon("./src/GUI/Affichage/Othello/1.gif");
        ...
        for (int i = 0; i < lignes; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                if (jeton != plateau.AUCUN_JOUEUR) {
                    display[i][j] = new JLabel(pion[jeton]);
                } else {
